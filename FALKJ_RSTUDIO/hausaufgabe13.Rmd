...
priming<-subset(priming,item<=20)
...
```
print(xtable(priming.by.subject),type="html",include.rownames=FALSE,results='asis')
...
All of the diagonal rows do not make much sense to the 
untrained eye.
...
The density graph makes the reader focus more on the "DD" 
and "ED" results, but the boxplot graph puts the focus more 
on the "DE" results.  The jitter graph is hard to interpret.  
The violin graph looks interesting and if the reader takes 
a little time to understand it, it may provide the best 
way to interpret the data most accurately.
...
ggplot(data=priming.by.item)+geom_jitter(aes(x=cond,y=RT,color=cond,fill=cond),alpha=0.5)
...
The data does not look the same on the graph, but it does 
look similar.  The density graph has the "DD" and "ED" at 
about the same height, but now the "DE" is higher than 
the "EE". The jitter graph is still difficult to interpret.  
The violin and boxplot graphs seem to show the information 
...
```{results='asis'}
library(ez)
priming.f2.deutsch.target <- ezANOVA(subset(priming, target == "D")
                                     , dv=.(RT)
                                     , wid=.(subj)
                                     , within=.(prime)
                                     , detailed=TRUE
...
```{results='asis'}
print(xtable(priming.f2.deutsch.target$ANOVA
             , display=c("s","s","d","d","f","f","f","fg","s","g")
             , digits=c(0,0,0,0,2,2,2,2,0,2))
      , type="html",include.rownames=FALSE)
```
